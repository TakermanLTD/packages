...
namespace Takerman.Mixer.Services.Providers
{
    ...
    public class PexelsProvider(IOptions<PexelsConfig> _pexelsOptions, IOptions<CommonConfig> _options) : BaseProvider, IPexelsProvider
    {
        public async Task Download(string search)
        {
            var urls = await GetUrls(search);

            var imageUrls = urls.OrderBy((item) => new Random().Next()).Take(_options.Value.NumOfClips).ToList();
            ...
            for (int i = 0; i < imageUrls.Count; i++)
            {
                ...
                using var fileStream = new FileStream(Path.Combine(_options.Value.MixLocation, "image_" + i + ".jpg"), FileMode.Create, FileAccess.Write, FileShare.None);
